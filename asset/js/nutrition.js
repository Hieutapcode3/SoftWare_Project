...
function displayFood(filteredData) {
    ...
    }, 100); // Đợi 300ms để hiệu ứng ẩn hoàn thành trước khi xóa nội dung
}
...
function sortByName() {
    ...
    rows.sort((a, b) => {
        ...
        return isAscendingName ? cellA.localeCompare(cellB) : cellB.localeCompare(cellA); // Sắp xếp tên
    });
...
document.addEventListener('click', function(event) {
    if (!event.target.closest('.header-container')) {
        ...
        sortIcons.forEach(icon => {
            icon.classList.remove('fa-arrow-up-wide-short', 'fa-arrow-down-wide-short','fa-arrow-down-a-z','fa-arrow-up-z-a');
